  ...
  <div class="toggle-box">
    ...
    <el-form v-else ref="form" :model="form" label-width="80px" style="padding: 15px">
      ...
      <el-form-item label="半径">
        <el-input v-model="form.radius" readonly></el-input>
      </el-form-item>
...
export default {
  components: {
    'my-feature': () => import('./map-feature/circle')
  },
  data() {
    return {
      idKey: 'circle',
  ...
  methods: {
    ...
    onsubmit() {
      ...
      feature.setCenter(this.form.center)
      feature.setRadius(this.form.radius)
      this.onback()
    },
    goAnchor(selector) {
    ...
    edit(item) {
      ...
      this.editor = new AMap.CircleEditor(this.map, this.feature)
      this.editor.on('move', e => this.form.center = [e.lnglat.lng, e.lnglat.lat])
      this.editor.on('adjust', e => this.form.radius = e.radius)
      this.editor.open()
    },
    ondel(item, index) {
