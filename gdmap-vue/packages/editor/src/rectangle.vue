<template>
  <lhp-amap-rectangle ref="overlay" v-bind="$props" v-on="$listeners" @onCreated="openEdit"></lhp-amap-rectangle>
</template>

<script>
import LhpAmapRectangle from 'lhp-amap/packages/rectangle'
...
let _default = config.getEditor('rectangle')

export default {
  name: 'LhpAmapEditorRectangle',
  mixins: [emitter],
  props: {
    ...LhpAmapRectangle.props,
    map: {
      required: true
    },
    options: {
