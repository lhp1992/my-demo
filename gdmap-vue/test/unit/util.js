...
export const loadAMapJs = function() {
  return new Promise((resolve, reject) => {
    if(document.head.querySelector('script[name="amap"]')) {
      setTimeout(resolve, 0);
      return
    }

    const script = document.createElement('script');
    script.setAttribute('name', "amap")
    script.type = 'text/javascript';
    script.async = true;
    script.defer = true;
    script.src = 'http://webapi.amap.com/maps?v=1.4.15&key=2d5b9876e9b0a4a522ced74a580450c9';
...
export const createAMap = async function() {
  await loadAMapJs()
  let vm = createTest({
    template: `
      <lhp-amap ref="amap" style="width: 1920px; height:1080px;"></lhp-amap>
    `
  }, true)
  await wait(1000)
  return vm.$refs.amap
