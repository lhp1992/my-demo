...
import Vue from 'vue'
import Layer from 'lhp-amap/packages/layer'
Vue.use(Layer)
...
describe('Layer', () => {
  ...
  const createLayer = function(props, data) {
    return util.createVue({
      template: `
        <lhp-amap-layer ref="layer" :map="map" ${props}></lhp-amap-layer>
      `,
      data() {
        return {
          map,
  ...
  it('layer unvalue toggle', async() => {
    vm = util.createTest(Layer, {map: map})
    await util.wait(300)
  ...
  it('disabled', async() => {
    vm = util.createTest(Layer, {map: map, disabled: true, alone: true})
    await util.wait(300)
  ...
  it('unvisibleIds', async() => {
    ...
    vm = util.createTest(Layer, {map: map, data: points, unvisibleIds: [points[0].id]})
    await util.wait(300)
    let $$cluster = vm.$refs.cluster.$$cluster
  ...
  describe('data', () => {
    ...
    describe('update item data', () => {
      let currentMarkers
      it('content', async() => {
        currentMarkers = $$cluster.getMarkers()
        vm.$set(vm.points[0], 'content', 'test content')
        await util.wait(300)
        expect(currentMarkers[0].getContent()).to.equal('test content')
      })

      it('position', async() => {
        vm.$set(vm.points[0], 'position', [112, 28])
        await util.wait(300)
        let position = currentMarkers[0].getPosition()
        expect(position.lng).to.equal(112)
        expect(position.lat).to.equal(28)
      })

      it('extData', async() => {
        let extData = currentMarkers[0].getExtData()
        expect(extData).to.eql(vm.points[0])
        util.destroyVM(vm)
      })
    })
  })
